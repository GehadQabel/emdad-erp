...
export default function DashboardPage() {
  ...
  // Workspace Titles
  const roleWorkspaceTitles: Record<string, { ar: string; en: string }> = {
    ADMIN: { ar: 'مساحة عمل الإدارة العامة والمؤشرات الاستراتيجية (Executive Admin)', en: 'Executive Operations Workspace' },
    WAREHOUSE: { ar: 'مساحة عمل إدارة المستودع والعمليات اللوجستية (Warehouse)', en: 'Warehouse Logistics Workspace' },
    SALES: { ar: 'مساحة عمل إدارة المبيعات والطلبيات (Sales)', en: 'Sales Pipeline Workspace' },
    PURCHASING: { ar: 'مساحة عمل إدارة المشتريات والتوريد (Purchasing)', en: 'Procurement Workspace' },
    FINANCE: { ar: 'مساحة عمل الإدارة المالية ومتابعة التحصيل (Finance)', en: 'Finance & Receivables Workspace' },
  }

  const currentWorkspace = roleWorkspaceTitles[role] || roleWorkspaceTitles.ADMIN
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ========================================================
          1. 🛡️ لوحة مدير النظام (ADMIN) - رؤية شاملة 360°
      ======================================================== */}
      {role === 'ADMIN' && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <KPICard title={locale === 'ar' ? 'مبيعات اليوم المحققة' : "Today's Sales"} value={formatCurrency(metrics.today_sales, locale)} subtitle={locale === 'ar' ? 'تم التسليم والفاتورة اليوم' : 'Delivered Today'} icon={DollarSign} colorVariant="emerald" href="/sales/orders" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-xl backdrop-blur-xl">
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <h2 className="text-sm font-bold text-white flex items-center gap-2"><Flame className="w-4 h-4 text-amber-400" /> {locale === 'ar' ? 'الأصناف الأكثر طلباً ومبيعاً' : 'Top Selling Products'}</h2>
              ...
              <div className="divide-y divide-slate-800/60 mt-2">
                {metrics.top_products?.length === 0 ? (
                  <p className="text-xs text-slate-500 py-6 text-center">{locale === 'ar' ? 'لا توجد مبيعات مسجلة حتى الآن' : 'No product sales recorded'}</p>
                ) : (
                  metrics.top_products?.map((p: any, idx: number) => (
                    <div key={idx} className="py-3 flex items-center justify-between text-xs">
                      ...
                      <div className="text-right rtl:text-left">
                        <p className="font-bold text-emerald-400 font-mono text-xs">{formatQuantity(p.total_qty_sold)} {locale === 'ar' ? 'كرتونة/عبوة' : 'units'}</p>
            ...
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-xl backdrop-blur-xl">
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <h2 className="text-sm font-bold text-white flex items-center gap-2"><Award className="w-4 h-4 text-sky-400" /> {locale === 'ar' ? 'كبار العملاء وسلاسل السوبرماركت' : 'Top Supermarkets'}</h2>
              ...
              <div className="divide-y divide-slate-800/60 mt-2">
                {metrics.top_customers?.length === 0 ? (
                  <p className="text-xs text-slate-500 py-6 text-center">{locale === 'ar' ? 'لا توجد تعاملات مسجلة حتى الآن' : 'No customer volume recorded'}</p>
                ) : (
                  metrics.top_customers?.map((c: any, idx: number) => (
                    <div key={idx} className="py-3 flex items-center justify-between text-xs">
                      <div className="flex items-center gap-3">
                        ...
                        <div><p className="font-bold text-white">{c.customer_name}</p><p className="text-[10px] text-slate-500">{c.total_orders_count} {locale === 'ar' ? 'طلبيات منفذة' : 'orders'}</p></div>
                      </div>
      ...
      {role === 'FINANCE' && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <KPICard title={locale === 'ar' ? 'مديونيات متأخرة واجبة التحصيل' : 'Overdue Debt'} value={formatCurrency(metrics.overdue_receivables, locale)} icon={AlertTriangle} colorVariant="crimson" href="/finance/receivables" />
            ...
            <KPICard title={locale === 'ar' ? 'عملاء محظورون لتأخر السداد' : 'Overdue Blocked Accounts'} value={`${metrics.blocked_cust_count} ${locale === 'ar' ? 'عميل محظور' : 'clients'}`} icon={ShieldAlert} colorVariant={metrics.blocked_cust_count > 0 ? 'crimson' : 'emerald'} href="/customers" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="divide-y divide-slate-800/60 mt-2">
                {metrics.urgent_receivables?.map((r: any, idx: number) => (
                  <div key={idx} onClick={() => router.push('/finance/receivables')} className="py-3 flex items-center justify-between text-xs cursor-pointer hover:bg-slate-800/30 px-2 rounded-xl transition-colors">
                    <div><p className="font-bold text-white">{r.customer_name}</p><p className="text-[10px] text-slate-500 font-mono">{r.receivable_number} • استحقاق: {r.due_date}</p></div>
                    <div className="text-right rtl:text-left"><p className="font-bold font-mono text-rose-400 text-sm">{formatCurrency(r.outstanding, locale)}</p>{r.is_overdue && <span className="text-[9px] text-rose-400 font-bold bg-rose-500/10 px-1.5 py-0.2 rounded border border-rose-500/20">{locale === 'ar' ? 'متأخرة' : 'Overdue'}</span>}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-xl">
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <h2 className="text-sm font-bold text-white flex items-center gap-2"><Coins className="w-4 h-4 text-emerald-400" /> {locale === 'ar' ? 'أحدث المقبوضات والتحصيلات' : 'Recent Collections'}</h2>
              ...
              <div className="divide-y divide-slate-800/60 mt-2">
                {metrics.recent_payments?.map((pay: any, idx: number) => (
                  <div key={idx} onClick={() => router.push('/finance/payments')} className="py-3 flex items-center justify-between text-xs cursor-pointer hover:bg-slate-800/30 px-2 rounded-xl transition-colors">
                    <div><p className="font-bold text-white">{pay.customer_name}</p><p className="text-[10px] text-slate-500 font-mono">{pay.payment_number} • {pay.payment_method}</p></div>
                    <div className="text-right rtl:text-left"><p className="font-bold font-mono text-emerald-400 text-sm">+{formatCurrency(pay.amount, locale)}</p><p className="text-[10px] text-slate-500">{pay.date}</p></div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* ========================================================
          3. 📦 لوحة أمين المستودع (WAREHOUSE) - بدون أسعار أو إيرادات!
      ======================================================== */}
      {role === 'WAREHOUSE' && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="divide-y divide-slate-800/60 mt-2">
                {metrics.warehouse_queue?.map((q: any, idx: number) => (
                  <div key={idx} onClick={() => router.push('/sales/orders')} className="py-3 flex items-center justify-between text-xs cursor-pointer hover:bg-slate-800/30 px-2 rounded-xl transition-colors">
                    <div><p className="font-bold text-white">{q.customer_name} — <span className="font-mono text-sky-400">{q.order_number}</span></p><p className="text-[10px] text-slate-400">{q.location_name}</p></div>
                    <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-indigo-500/10 text-indigo-400 border border-indigo-500/30">{q.status}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-xl">
              ...
              <div className="divide-y divide-slate-800/60 mt-2">
                {metrics.critical_stock?.map((item: any, idx: number) => (
                  <div key={idx} onClick={() => router.push('/inventory/balances')} className="py-3 flex items-center justify-between text-xs cursor-pointer hover:bg-slate-800/30 px-2 rounded-xl transition-colors">
                    <div><p className="font-bold text-white">{item.product_name}</p><p className="text-[10px] text-slate-400">{item.warehouse_name} • {item.product_code}</p></div>
                    <div className="text-right rtl:text-left font-mono"><span className="text-xs font-bold text-amber-400">{formatQuantity(item.on_hand_qty)} رصيد حالي</span><span className="text-[10px] text-slate-500 block">حد النواقص: {formatQuantity(item.min_stock_level)}</span></div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {role === 'PURCHASING' && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="divide-y divide-slate-800/60 mt-2">
                {metrics.reorder_items?.map((item: any, idx: number) => (
                  <div key={idx} onClick={() => router.push('/purchasing/orders')} className="py-3 flex items-center justify-between text-xs cursor-pointer hover:bg-slate-800/30 px-2 rounded-xl transition-colors">
                    <div><p className="font-bold text-white">{item.product_name}</p><p className="text-[10px] text-slate-400">{item.warehouse_name} • {item.product_code}</p></div>
                    <div className="text-right rtl:text-left font-mono"><span className="text-xs font-bold text-amber-400">{formatQuantity(item.on_hand_qty)} رصيد</span><span className="text-[10px] text-slate-500 block">حد الأمان: {formatQuantity(item.min_stock_level)}</span></div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-xl">
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <h2 className="text-sm font-bold text-white flex items-center gap-2"><Truck className="w-4 h-4 text-sky-400" /> {locale === 'ar' ? 'أوامر الشراء المفتوحة ومواعيد الوصول' : 'Open Purchase Orders'}</h2>
                <button onClick={() => router.push('/purchasing/orders')} className="text-xs text-sky-400 font-semibold">{locale === 'ar' ? 'عرض الكل' : 'View All'}</button>
              </div>
              <div className="divide-y divide-slate-800/60 mt-2">
                {metrics.open_pos?.map((po: any, idx: number) => (
                  <div key={idx} onClick={() => router.push('/purchasing/orders')} className="py-3 flex items-center justify-between text-xs cursor-pointer hover:bg-slate-800/30 px-2 rounded-xl transition-colors">
                    <div><p className="font-bold text-white">{po.supplier_name}</p><p className="text-[10px] text-slate-500 font-mono">{po.po_number} • استلام متوقع: {po.expected_date}</p></div>
                    <div className="text-right rtl:text-left"><p className="font-bold font-mono text-white text-sm">{formatCurrency(po.total_amount, locale)}</p>{po.is_late && <span className="text-[9px] text-rose-400 font-bold bg-rose-500/10 px-1.5 py-0.2 rounded border border-rose-500/20">{locale === 'ar' ? 'متأخر' : 'Late'}</span>}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {role === 'SALES' && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <KPICard title={locale === 'ar' ? 'طلبيات معتمدة ومحجوزة بالمخزن' : 'Approved Orders'} value={`${metrics.orders_to_prepare} ${locale === 'ar' ? 'طلبية' : 'orders'}`} icon={CheckSquare} colorVariant="sky" href="/sales/orders" />
            <KPICard title={locale === 'ar' ? 'طلبيات جاهزة للتسليم للعملاء' : 'Ready for Delivery'} value={`${metrics.orders_ready} ${locale === 'ar' ? 'شحنة' : 'ready'}`} icon={Truck} colorVariant="purple" href="/sales/orders" />
